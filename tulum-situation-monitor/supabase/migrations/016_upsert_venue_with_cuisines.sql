-- Update upsert_venue_from_google to automatically tag cuisines
...
DECLARE
  ...
  v_cuisines JSONB;
BEGIN
  -- Auto-tag cuisines for restaurants and cafes
  IF p_category IN ('restaurant', 'cafe') THEN
    v_cuisines := public.infer_cuisines(
      p_name,
      COALESCE((p_google_data->'types')::TEXT[], ARRAY[]::TEXT[]),
      p_category
    );
  ELSE
    v_cuisines := '[]'::jsonb;
